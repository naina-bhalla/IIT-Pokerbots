    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if street == "pre-flop":
            ...
            if is_sb and cost <= 10:
                if equity > 0.52 and current_state.can_act(ActionRaise):
        ...
        if street == "auction":
            ...
            if equity > 0.75:
                bid = original_pot * 0.6
            elif equity > 0.55:
                bid = original_pot * 0.35
            elif equity > 0.40:
                bid = original_pot * 0.15
            else:
                bid = 0
            return ActionBid(int(min(bid, current_state.my_chips * 0.20)))
        ...
        pot_commit_ratio = cost / stack if stack > 0 else 1
        ...
        if self.opponent_archetype == "STATION":
            if equity > 0.62 and current_state.can_act(ActionRaise) and not is_betting_war:
                target = int(pot * random.uniform(0.65, 0.90))
                return ActionRaise(max(min_raise, min(target, max_raise)))

        if self.opponent_archetype == "NIT" and not is_betting_war:
            if equity < 0.45 and current_state.can_act(ActionRaise) and random.random() < 0.45:
                target = int(pot * random.uniform(0.5, 0.65))
                return ActionRaise(max(min_raise, min(target, max_raise)))
        ...
        if equity > 0.80 and current_state.can_act(ActionRaise):
            if spr < 2:
                return ActionRaise(max_raise)
            target_raise = int(pot * random.uniform(0.65, 0.85))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))

        if equity > 0.63 and not is_betting_war and current_state.can_act(ActionRaise):
            target_raise = int(pot * random.uniform(0.35, 0.55))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))

        if opp_checked and equity > 0.45 and current_state.can_act(ActionRaise):
