...
class Player(BaseBot):
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if 0.63 < equity <= 0.82 and not is_betting_war and current_state.can_act(ActionRaise):
            target_raise = int(pot * random.uniform(0.35, 0.50))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))


        if opp_checked and 0.45 < equity <= 0.63 and current_state.can_act(ActionRaise):
            target_raise = int(pot * random.uniform(0.25, 0.40))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))
