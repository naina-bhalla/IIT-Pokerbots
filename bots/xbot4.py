...
class Player(BaseBot):
    def __init__(self):
        ...
        self.reveal_applied = False

        self.opp_streets_bet_this_hand = 0
        self.opp_preflop_raise_count = 0
        self.opp_preflop_fold_count = 0
    ...
    def _get_iters(self, base_iters):
        if self.time_bank > 20: return base_iters
        if self.time_bank > 10: return int(base_iters * 0.75)
        if self.time_bank > 5:  return int(base_iters * 0.5)
        if self.time_bank > 2:  return int(base_iters * 0.25)
        return 10
    ...
    def filter_range_by_reveal(self, revealed_cards):
        for card_str in revealed_cards:
            for combo in list(self.range_weights.keys()):
                if card_str not in combo:
                    self.range_weights[combo] = 0
        self._normalize_range()

    def bayesian_update_check(self, board, street):
        if not board:
            return
        board_eval = [eval7.Card(c) for c in board]
        board_strs = set(board)
        ...
        min_score = strengths[-1][1]
        max_score = strengths[0][1]
        check_factor = {"flop": 0.4, "turn": 0.6}.get(street, 0.8)
        for combo, score in strengths:
            strength_norm = (score - min_score) / (max_score - min_score) if max_score != min_score else 0.5
            likelihood = 1.0 - check_factor * strength_norm
            self.range_weights[combo] *= likelihood
        self._normalize_range()
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if cost > 0:
            ...
            if street in ("flop", "turn", "river"):
                self.opp_streets_bet_this_hand += 1
        ...
        if street == "pre-flop":
            ...
            is_sb = not current_state.is_bb

            invested = pot - cost if is_sb else pot - cost
            stack = current_state.my_chips + invested
            pot_commitment = cost / stack if stack > 0 else 0

            if pot_commitment > 0.15:
                if equity < 0.62:  
                    if current_state.can_act(ActionCall) and equity > pot_odds + 0.05:
                        return ActionCall()
            ...
            if cost > 0 and is_sb:
                self.opp_preflop_raise_count += 1


            if is_sb and cost <= 10:
                if equity > 0.50 and current_state.can_act(ActionRaise):
                    lo, hi = current_state.raise_bounds
                    return ActionRaise(max(lo, min(int(pot * 2), hi)))
                if equity > 0.33:
            ...
            if current_state.is_bb and cost > 0:
                if equity > 0.62 and cost <= 40 and current_state.can_act(ActionRaise):
                    lo, hi = current_state.raise_bounds
                    return ActionRaise(max(lo, min(int(pot * 1.5), hi)))
        ...
        if street == "auction":
            ...
            if equity > 0.70:
                bid = original_pot * 0.65
            elif equity > 0.55:
                bid = original_pot * 0.40
            elif equity > 0.40:
                bid = original_pot * 0.25
            elif equity > 0.30:
                bid = original_pot * 0.08
            else:
                bid = 0
            return ActionBid(int(min(bid, current_state.my_chips * 0.25)))


        opp_revealed = current_state.opp_revealed_cards
        if opp_revealed and not self.reveal_applied:
            self.filter_range_by_reveal(opp_revealed)
            self.reveal_applied = True


        opp_checked = (not current_state.is_bb and cost == 0 and street in ("flop", "turn", "river"))
        if opp_checked:
            self.bayesian_update_check(board, street)

        # === POSTFLOP ===
        base_iters = 500 if street == "river" else 200
        equity = self.compute_equity(my_cards, board, base_iters)
        pot_odds = cost / pot if pot > 0 else 0
        ...
        has_reveal = self.reveal_applied and bool(opp_revealed)
        info_edge = 0.03 if has_reveal else 0.0

        barrel_penalty = self.opp_streets_bet_this_hand * 0.06

        stack = current_state.my_chips

        # Archetype exploits
        if self.opponent_archetype == "MANIAC":
            if true_bet_ratio > 1.5 and equity < 0.82:
                return ActionFold()
            if equity > 0.72 and current_state.can_act(ActionCall):
                return ActionCall()

        if self.opponent_archetype == "STATION":
            if equity > 0.58 and current_state.can_act(ActionRaise) and not is_betting_war:
                target = int(pot * random.uniform(0.65, 0.95))
                return ActionRaise(max(min_raise, min(target, max_raise)))

        if self.opponent_archetype == "NIT" and not is_betting_war:
            if equity < 0.45 and current_state.can_act(ActionRaise) and random.random() < 0.50:
                target = int(pot * random.uniform(0.50, 0.70))
                return ActionRaise(max(min_raise, min(target, max_raise)))

        # Multi-barrel defense
        if self.opp_streets_bet_this_hand >= 2 and equity < 0.58 + barrel_penalty:
            if cost > 0:
                if current_state.can_act(ActionCheck):
                    return ActionCheck()
                return ActionFold()

        # === RIVER-SPECIFIC (equity is exact, no more cards to come) ===
        if street == "river":
            if equity > 0.92 and current_state.can_act(ActionRaise):
                target = int(pot * random.uniform(0.80, 1.15))
                return ActionRaise(max(min_raise, min(target, max_raise)))
            ...
            if equity > 0.65 - info_edge and current_state.can_act(ActionRaise):
                target = int(pot * random.uniform(0.55, 0.80))
                return ActionRaise(max(min_raise, min(target, max_raise)))

            # River bluff when checked to with weak hands
            if opp_checked and equity < 0.28 and random.random() < 0.18:
                if current_state.can_act(ActionRaise):
                    target = int(pot * random.uniform(0.55, 0.75))
                    return ActionRaise(max(min_raise, min(target, max_raise)))
        ...
        # === FLOP / TURN ===
        if is_betting_war:
            if equity > 0.90 and current_state.can_act(ActionRaise):
                target_raise = int(pot * random.uniform(0.7, 1.0))
                return ActionRaise(max(min_raise, min(target_raise, max_raise)))
            if equity > pot_odds + 0.06 + barrel_penalty and current_state.can_act(ActionCall):
                return ActionCall()
            if current_state.can_act(ActionCheck):
                return ActionCheck()
            return ActionFold()

        if equity > 0.80 - info_edge and current_state.can_act(ActionRaise):
            if spr < 2:
                return ActionRaise(max_raise)
            target_raise = int(pot * random.uniform(0.65, 0.85))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))

        if equity > 0.60 - info_edge and not is_betting_war and current_state.can_act(ActionRaise):
            target_raise = int(pot * random.uniform(0.35, 0.55))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))

        if opp_checked and equity > 0.42 - info_edge and current_state.can_act(ActionRaise):
            target_raise = int(pot * random.uniform(0.30, 0.45))
            return ActionRaise(max(min_raise, min(target_raise, max_raise)))

        # Bluff probe when checked to with weak hands
        if opp_checked and 0.12 < equity < 0.35 and random.random() < 0.22:
            if current_state.can_act(ActionRaise):
                target = int(pot * random.uniform(0.45, 0.65))
                return ActionRaise(max(min_raise, min(target, max_raise)))

        if true_bet_ratio > 1.5 and equity < 0.72:
            return ActionFold()

        call_threshold = pot_odds + 0.03 + barrel_penalty
        if equity > call_threshold and current_state.can_act(ActionCall):
            return ActionCall()
